...
class ServiceValue {
  ...
  Future<List<ServiceValue>> generateInputValues({required List<ServiceInput>? serviceInputs,required int serviceId}) async {
   List<ServiceValue> serviceValues=[];
    for (var i = 0; i < serviceInputs!.length; i++) {
      serviceValues.add( ServiceValue( inputservice_id:serviceInputs[i].id,selectedservice_id: serviceId));
    }
